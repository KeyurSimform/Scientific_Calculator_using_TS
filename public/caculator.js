...
const input_elements_selectors = document.querySelector(".input"); /*global constant to select the input tags */
const output_operations_selectors = document.querySelector(".operation .value"); /*global constant to select the operations visible in the calculators display */
const output_result_selectors = document.querySelector(".result .value"); /*global constant to select the final result value for the calculator*/
...
function powerBaseFetcher(formula, power_search_result) {
    ...
    power_search_result.forEach(power_index => {
        ...
        while (prev_index >= 0) {
            ...
            let is_apower = false;
            if (formula[prev_index] == "(")
                parantheses_count--;
            if (formula[prev_index] == ")")
                parantheses_count++;
            OPERATORS.forEach(OPERATOR => {
                if (formula[prev_index] == OPERATOR) {
                    is_aoperator = true;
                }
            });
            is_apower = formula[prev_index] == POWER;
            if ((is_aoperator && parantheses_count == 0) || is_apower)
                break;
...
function FactorialNumberFetcher(formula, factorial_search_result) {
    ...
    factorial_search_result.forEach(factorial_index => {
        ...
        while (prev_index >= 0) {
            ...
            if (is_aoperator && parantheses_count == 0)
                break;
